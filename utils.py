...
def train_student(model, optimizer, criterion_bce, criterion_kd, args, data, save_name, soft_target):
    ...
    for epoch in range(args.epochs):
        ...
        h, output, ib_loss = model(data.features, data.edge_index)
        ...
        loss_train = lad1 * loss_bce_train + lad2 * loss_kd_train + ib_loss
        ...
        model.eval()
        with torch.no_grad():
            h, output,_ = model(data.features, data.edge_index)
